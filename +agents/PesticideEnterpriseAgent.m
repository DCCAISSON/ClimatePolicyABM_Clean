...
classdef agents.agents
    % PesticideEnterpriseAgent 农药企业智能体
    % 继承自EnterpriseAgent，代表农药生产企业。每个企业只生产一种农药，但企业间可有异质性。
    % 经济学机制：利润最大化、成本函数、环境风险、监管合规、有限理性等。
    ...
    properties
        ...
        policy         % 政策参数结构体（如排放税率、监管罚款等）
        % 新增：质量相关属性
        reputation     % 企业声誉
        r_and_d_investment % 研发投资
    end
    methods
        function obj = PesticideEnterpriseAgent(id, params, spatial_grid, policy)
            % 构造函数，初始化农药企业属性
            obj@EnterpriseAgent(id, params, spatial_grid);
            obj.type = 'industrial';
            obj.subtype = 'pesticide';
            obj.emission_rate = 0.05 + 0.15 * rand;
        ...
        function cost = calculate_total_cost(obj, output)
            % 总成本 = 固定成本 + 变动成本 + 排放税 + 合规罚款 + 质量投资成本
            fixed_cost = 1200;
            variable_cost = output * (1.7 - 0.6 * obj.technology_level);
            emission_tax = output * obj.emission_rate * obj.policy.emission_tax_rate;
            compliance_penalty = output * (1 - obj.technology_level) * obj.policy.compliance_penalty_rate;
            
            % 新增：质量投资成本
            quality_cost = obj.revenue * obj.quality_investment;
            rd_cost = obj.revenue * obj.r_and_d_investment;
            
            cost = fixed_cost + variable_cost + emission_tax + compliance_penalty + quality_cost + rd_cost;
        end
        
        function mc = calculate_marginal_cost(obj)
            % 边际成本
            mc = 1.7 - 0.6 * obj.technology_level + obj.emission_rate * obj.policy.emission_tax_rate + ...
                 (1 - obj.technology_level) * obj.policy.compliance_penalty_rate;
        end
